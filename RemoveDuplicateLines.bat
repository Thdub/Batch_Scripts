...
:: Remove duplicates lines from any text file.
:: Created by Th.dub @ResonantStep 2019
:: All credits goes to dbenham for his awesome JRepl.bat utility
:: Just drag and drop your text file on to this script.
:: JREPL.BAT v8.2 by Dave Benham

@echo off
if exist "%TEMP%\lock_process.tmp" ( goto :Batch) else ( echo "%~dpnx1">"%TEMP%\lock_process.tmp")
set /p File2edit=<"%TEMP%\lock_process.tmp"
"%~dpnx0" "^(.*)(?:\r?\n|\r)(?=[\s\S]*^\1$^)" "" /m /f "%~dpnx1" /o - >NUL 2>&1
...
PowerShell -NoProfile -ExecutionPolicy Bypass "(Get-Content '%File2edit%') | Set-Content '%File2edit%'"
